...
9. 半自动买入 (execution.mode=semi_auto, 用户 2026-07-20 设立, 取代原 confirm 闸门): 实盘新买入与配套换仓卖单**不得**在无人值守会话中 review/place (会被平台分类器拦截, 不要反复尝试) — 只能由主流程写入 `state/pending_orders.json` (逐字段来自引擎输出), 待用户在有人值守会话明确说"执行"后按 playbook 4C 原样执行, 当日 15:55 ET 过期。出场/止损/兜底卖出与纸面轨道不受限, 照常全自动。
