...
9. 确认闸门 (execution.mode=confirm, 用户 2026-07-16 设立): 实盘新买入与配套换仓/加速清理卖单必须有当日有效的用户确认 (state/approval.json approved=true 且标的在候选内; 加速清理卖单标的须在 funding_sell_order 内); 出场/止损卖出与纸面轨道不受限。无确认 → 只出不进。
