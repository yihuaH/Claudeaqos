...
9. 半自动买入 (execution.mode=semi_auto, 用户 2026-07-20 设立, 取代原 confirm 闸门): 实盘新买入与配套换仓/加速清理卖单 (funding_rotation / accelerated_liquidation) 在无人值守会话**可 review、不得 place** (平台分类器只拦截无人值守 place, review 不受限; 不要反复尝试下单) — 只能由主流程写入 `state/pending_orders.json` (逐字段来自引擎输出), 待用户在有人值守会话明确说"执行"后按 playbook 4C 原样执行 (**2026-09-10 用户批准换协议**: 次一交易日 **09:45–15:55 ET 盘中单笔 market+regular_hours+dollar_amount**, 推荐锚 10:45 晨检; **必须在 09:30 开盘之后** —— 出场回款 09:30 到账而原清单 09:25 过期, 使引擎算进去的回款结构性拿不到, 实测占出单额 32%/日均 $1,277; 单笔市价原生支持分数股; 不设价格闸; 隔夜轨道买单仍仅当日)。**2026-09-25 用户选「3」部分恢复盘外腿 —— 执行形态现为双模式**: **(a) 盘中 09:45–15:55 ET = 单笔 market+regular_hours+dollar_amount, 首选、零额外成本**; **(b) 盘外 15:55 后至次一交易日 09:45 (含周末) = 整股限价①腿** (`limit = 引擎 est_price × 1.010`, `all_day_hours→extended_hours→regular_hours` 降级; `whole==0` 该单整单跳过留给次日; **零头不走旧②腿**, 随清单既有顺延机制走次日盘中窗 —— 由此规避原 09:25 截止「回款结构性拿不到」缺陷)。盘外①腿实测**多付 0.55%** + 盘后点差中位约 2%, 属**有成本的退路非首选**; ⚠️ 走盘外时**必须重做 4C-1B 逐批确认** (订单形态从美元额市价变为整股限价, 数量不同且可能有单落空)。触发依据: 5 个交易日内越窗两次 (09-21 / 09-25)。出场/止损/兜底卖出与纸面轨道不受限, 照常全自动 (可直接 place)。
...
| 周度期权实盘实验仓 (**credit_put_spread** 卖0.97P/买0.88P, 2026-08-13 用户「直接实盘」换形态, 验证期重置) | 实盘 | ✅ active·机会主义 (「有合适就买, 没有就不买」; budget **不设上限** 与单张风险 **不设上限** (2026-08-18 用户「不设上限」, 由 50%/$5,000 取消); 仍生效: 实时 BP 双封顶 (红线3 防杠杆闸)·熔断 (累计已实现亏 ≥ 净值×50%, 独立配置未动)·注码=净值×20% 风险/信号·**贵档单仓亦不设上限** (2026-08-18 用户「取消掉」, 三闸全撤)·点差闸·DTE闸·**公允价边际闸** (`contract.min_est_vs_model_pct=90`, 2026-09-03 用户回「b」批准: credit 形态要求保守净贷记 ≥ 参考公允价×90%, 起因 09-01 XLF 单 est 仅为 model 的 32%/风险收益比 249:1; 缺省关闭, paper 轨道未设阈值故闸不生效。**2026-09-11 用户回「b」批准 B 方案**: 参考价由 `contract.model_edge_reference` 选 — `"flat"` (缺省, = 原 RV×1.25 两腿同 IV 口径, **行为与上线以来完全一致**) 或 `"surface"` (`model.surface` 实测 IV 曲面, 逐腿按行权价取 IV)。两个参考价**无论如何都并行计算并写进 skips/buys 的 `model_edge` 字段** (影子模式), 攒够样本后由用户决定是否切换; 起因: 90% 常数对上「公平比值」54%(TSLA)~252%(SPY) 的区间, 9/21 只即使报价公允也被拦、8/21 只几乎永不触发)。⚠️ 期权规模唯一硬约束现为**实时 buying_power**, 单笔在险额可超净值一半 (实测 AVGO 一张 = 净值 60.2%), 用户知情接受; 白名单21只·股票类专属; 开仓/平仓均**手动通道** (2026-08-15 实测 agentic 暂不支持多腿 place, 引擎出单+对话发参数+用户 App 执行, 详 playbook 4D 注记; 08-17 首笔 $1.53 成交)) | `weekly_calls_live.json enabled` + `budget` |

## 结构

- `strategy/config.json` — 策略与风控参数 (用户可改)
- `strategy/playbook.md` — **每日必读的执行契约** (前置检查/§4 下单协议 4A-4D/回写/次日预览/异常原则; 2026-08-06 拆分, 2026-09-10 4C 换代后 417 行)
...
- `strategy/archive/retired-offhours-two-leg.md` — 已退役的 4C 盘外两腿混合执行协议全文 (2026-07-20→2026-09-10) + 退役依据 + 回退步骤
...
- `scripts/weekly_calls.py` — 周度期权确定性引擎 (signal / apply / report; 2026-08-04 用户授权), RSI-2 同形信号 × 跟正股出场, 支持 single / vertical_spread / **credit_put_spread** (2026-08-13 用户「直接实盘」, 实盘现行形态: 卖0.97P/买0.88P 收贷记, 每张风险=宽度−贷记, 点差双档闸 pct|abs; 另有**公允价边际闸** `model_edge_gate`, 按方向对称 — credit 要 est ≥ model×r、debit 要 est ≤ model÷r, 配置缺字段=关闭; 2026-09-11 起双参考价并行 (`iv_at` 逐腿 IV + `model.surface` 实测曲面), 由 `contract.model_edge_reference` 选用, 缺省 `flat` 行为不变), 双轨共用:
