...
候选池 = 9 只低价孪生 ETF (SPLG/QQQM/IWM/XLK/XLF/XLE/SOXQ/GLDM/TLT) + 100 只个股
(`screen.py` 周度筛选, 防御层强制: 财报回避/异动过滤/行业上限, 2026-07-21 起并入实盘);
同时纳管账户存量持仓(保护性止损 + 弱势轮动换仓)。
...
| 存量持仓 | 跌破纳管价 -7% 保护性止损; 盈利 (≥纳管价) 且反弹到位 (收盘>SMA5 或 RSI2>65) 止盈; 需要资金时按最弱者轮出/加速清理 |
...
- **每日战报**由绑定用户"报告窗口"会话的 Routine 发送 (`Claudeaqos 每日战报 → 报告窗口`), 每交易日两次:
  ...
  - **买单就绪推送 (~15:45 ET)** — 主流程生成待执行买单 (`state/pending_orders.json`) 时直接推送通知, 用户**次日开盘前任意时间**到报告窗口回复「执行」: 当日 15:55 ET 前为市价单, 之后自动转 24h 时段整股限价单 (盘后/隔夜/盘前皆可成交, 信号价 +1.0% 容差封顶, 跳空不追);
  - **16:45 ET 收盘战报** — 必发: 当日 journal 摘要 (净值/回撤/信号/成交/告警)、券商订单核对、待执行订单状态 (executed/expired); 主流程未产出时先复查 20 分钟再诊断上报。
- **买入半自动** (2026-07-20 起): 无人值守会话的买单 place (下单) 会被平台分类器拦截 (review 不受限), 故新买入与换仓卖单由主流程写入待执行清单, 用户在报告窗口回复「执行」后原样执行 (playbook 4C); 出场/止损卖单不受影响, 照常全自动 (可直接下单)。原 approval.json 每日预审确认制随之退役。
...
| Alpaca paper API | 市场时钟/假期日历 (休市判定) + 报价/日线/快照 | ✅ 已注入生效 (2026-07-22 确认), 已接入 playbook |
| FRED API | 宏观风控: VIX ≥ 30 暂停新开仓 | ✅ 已注入生效 (2026-07-22 确认), 过滤逻辑已进引擎 |
