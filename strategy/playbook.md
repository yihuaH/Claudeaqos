...
2. **盘中单笔市价模式 (2026-09-10 用户批准, 取代原「盘外两腿混合执行」)** — 按 seq 顺序执行
   ...
   - **执行窗口: 次一交易日 09:45–15:55 ET (= 06:45–12:55 PT), 推荐锚在 10:45 ET 晨检窗口。**
     必须**在 09:30 开盘之后**执行 —— 这是本次改动的核心 (见下"为什么"), 09:30 前一律不执行。
     用户可在窗口内任意时刻触发; 15:55 ET 后未执行 → status 改 `expired`, 引擎当晚重算。
