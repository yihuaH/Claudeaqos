...
> | 段 | 时刻 | 跑什么 | 产出 |
> |---|---|---|---|
> | **① 盘前关键路径** | **15:20 ET** (`daily.py --phase preclose`) | preflight → 取数 → 正股信号 → 期权信号 | 出场卖单**即时成交** + pending (首选窗 15:30–15:55, 未执行顺延次日 09:45–15:55) |
...
> 只把**时间敏感的那一小段**放盘前 (估 3–5 分钟), 纸面轨道/行情核对/journal 留在 17:45,
> 关键路径比 2026-07-24 退役的旧盘前主跑短一个数量级 (当初是整个主跑塞进 35 分钟才频繁挂起)。
>
> **⚠️ 盘前当日价来自券商实时报价, 不是 Alpaca**: `signals.py` 本就**排除当天日线**、
> 把当日价从 `--quotes` 取 (`signals.py:183-189`)。盘前必须传
...
>
> **fail-safe (跑挂了怎么办)**: 盘前跑成功会落 `state/preclose_status.json`。
> 17:45 的 `--phase wrapup` 读它 —— **当日没有 completed 标记就自动退化为完整主跑**
> (出场卖单照下、pending 按次日 09:45–15:55 窗口)。所以盘前挂掉最坏退回改动前的行为,
...
3. **幂等 (2026-09-10 重写, 原实现两个缺陷都已修)**: 驱动器扫当日**全部** `journal/<今天>*.md`,
   ...
   | 阶段 | 认的标记 (必须独立成行) | 由谁写 |
   |---|---|---|
   | `--phase preclose` | `preclose: completed` | 盘前跑收尾时写 |
   | `--phase wrapup` / `full` | `status: completed` | 收盘后收尾时写 (= 当日全流程结束) |
   | (晨检窗口) | `morning_check: completed` | 晨检写; 与上两者互不干扰 |

   **写 journal 时务必用对自己阶段的键** —— 盘前若误写 `status: completed`, 同日 17:45 的
...
1. **MCP 取数** (驱动器碰不到, 必须会话做):
   ...
   - **券商官方收盘 (行情管道交叉核对, 2026-08-08 用户批准, 每日必做)**: 对**持仓 + 当日买单候选**
     ...
     **仅 wrapup/full 阶段做** —— 盘前阶段官方收盘还不存在, 驱动器会自动跳过核对。
   - **⚠️ 盘前阶段 (`--phase preclose`) 专属: 当日实时价 (不做则信号全错)**:
     ...
     **绝不可让盘前阶段用 `integrations.py quotes`** —— 它走 `EQUITY_RT_FEED=delayed_sip`,
     延迟 15 分钟, 15:20 跑会拿到 15:05 的价 (CLAUDE.md 已警示"引入盘中决策须重评")。
     券商报价是实时的, 这是盘前唯一合规的当日价来源。
     > 驱动器有两道硬闸 (都会让盘前阶段直接 fatal, 不是 warn):
     > ① **盘前未传 `--quotes`** → 拒跑 (否则会静默退回 delayed_sip);
     > ② **覆盖率 <90%** → 拒跑。缺的标的 `signals.py` 会 warn「无实时报价, 使用最后一根历史K线」,
     >    而那根是**昨日**收盘, 该票当日信号即失真, 大面积失真等于信号全错。
     > 触发任一闸 → 按红线6 补齐报价重跑, 或直接放弃盘前段 (17:45 wrapup 会 fail-safe 兜住)。
2. **跑驱动器**:
   ```bash
   ...
   **`--phase` 必须传** (2026-09-10 拆分后):
   - **15:20 ET 盘前**: `--phase preclose` + `--quotes <scratchpad>/rh_quotes.json` (券商实时价, 见上)。
...
- **时效字段一律照抄 `plan.json` 的 `to_pending.pending_template`** (2026-09-10 加, 起因当日
  ...
  两种形态:
  - **盘前产出** (`exec_day: same_day_then_next_session`, 2026-09-11 用户选「顺延」):
    首选窗 **当日 15:30–15:55**; 未执行则**顺延**至**次一交易日 09:45–15:55**, 至次日 15:55 才过期。
    出场卖单已在盘前即时成交、回款已到账。**顺延期内 status 保持 `awaiting_execution`, 不标 expired。**
...
2. **盘中单笔市价模式 (2026-09-10 用户批准, 取代原「盘外两腿混合执行」)** — 按 seq 顺序执行
   ...
   - **执行窗口: 以清单自带的 `pending_template.exec_window_et` / `exec_day` 为准**
     ...
     - **`same_day_then_next_session` (盘前主跑产出; 2026-09-11 用户选「顺延」)**:
       首选窗当日 **15:30–15:55 ET** (= 12:30–12:55 PT) —— 出场卖单已在盘前即时成交、
       回款已到账, 所以本形态**不需要等次日开盘**。
       **当日窗内未执行 → 不作废, 顺延至次一交易日 09:45–15:55 ET** (与 next_session 同窗),
       次日 15:55 仍未执行才改 `expired`。
       - **起因**: 盘前窗仅 25 分钟, 2026-09-11 用户回复落在窗外, 4 单全部过期 0 成交
         (`journal/2026-09-11-preclose.md` §7)。顺延不缩短任何窗口, 只给漏掉的清单一条退路。
       - **不会重复买入**: 次一交易日盘前主跑产出新清单时**覆写** `pending_orders.json`,
         旧清单自动消失, 不存在两份并存。
...
2D-0. **盘前阶段的跨轨道次序 (2026-09-11 起, 两轨同窗口)**: 盘前产出时股票窗
...
- **买单 (buy_to_open, 绝不无人值守 place)**: 写入 `state/pending_option_orders.json`
  ...
  **时效: 以清单自带的 `option_pending_template` 为准** (2026-09-11 用户「期权也可以当日马上执行」
  后有两种形态, 与股票一样不得凭记忆):
  - **`same_day` (盘前主跑产出)**: 当日 **15:30–15:45 ET** (= 12:30–12:45 PT)。
...
2. 写 journal: 文件名取 `journal/<今天>.md`, **若已被当日其他窗口占用则加后缀**
   ...
   盘前 → `preclose: completed`; 收盘后收尾 → `status: completed`; 晨检 → `morning_check: completed`。
