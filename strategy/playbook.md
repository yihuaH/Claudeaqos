...
> **两个运行入口** (2026-07-23):
> - **主流程 (15:30 ET, 首选)**: 本文正文 §0–§8B。收盘前跑, 当天窗口内 4A 卖单市价成交、买单当日市价/盘外限价。
> - **盘后补跑 (17:45 ET, 兜底)**: 见 **§0C**。仅当主流程当日**未完成交易评估**时启用 (如 worker 重启错过窗口)。
>   用**最终收盘价**重算 (信号反而更准), 全程走异步执行 (4A 卖单市价排次日开盘, 买单入 pending 待用户「执行」)。
>   目的: 单个窗口挂掉不再等于白扔一天。执行本就 24h (all_day 限价), 补跑只补"算信号+挂单"这一步。

## 0C. 盘后补跑窗口 (catch-up, 主流程失败兜底)

独立 Routine ~17:45 ET (21:45 UTC) 每交易日唤醒。**条件运行, 正常日静默**:

0. **是否需要补跑** (git fetch origin Main 后判断):
   - `journal/<今天>.md` 存在且 `status: completed` (主流程已完成交易评估) → **不动作, 静默结束**。
   - `journal/<今天>-catchup.md` 已存在 (本补跑今日已跑过) → **幂等, 静默结束**。
   - `journal/<今天>.md` 不存在, 或存在但 status 为 closed/error 且注明"时段错过/未交易/worker/中断"等未完成交易评估 → **执行补跑** (下列步骤)。
   - 若当天为休市 (Alpaca 时钟) → 静默结束。
1. **时段校验**: 当前须在**收盘后**至次一交易日开盘前 (16:00 ET 当日 – 次日 09:25 ET); 收盘前误触发 → 只读结束 (让主流程走正流程)。
2. 取数/信号: 同 §1–§3, 但用**当日最终收盘价** (盘后 Alpaca/Robinhood 收盘报价), `--date` 仍为当日。
3. **执行 (全异步, 因已过收盘)**:
   - **4A 出场/止损/兜底卖单** (含 legacy_take_profit): 直接 place, 但 **market + regular_hours** (盘后提交自动排队次日开盘成交); 分数股用 market (可分数), 整股亦可。风险只减不增, 无需确认。
   - **4B 买单 + funding_rotation + accelerated_liquidation**: 绝不 place, 写入 `state/pending_orders.json` (valid_until 次日 09:25 ET), commit+push 后 PushNotification 通知用户到报告窗口回复「执行」(用户任意时间执行, 走 all_day 限价)。
4. 回写 (仅实际成交的 4A 卖单) `signals.py apply`; 隔夜/纸面/学习轨道同主流程 §5B–§7B 处理 (若主流程当日完全未跑)。
5. 写 `journal/<今天>-catchup.md` (**独立文件名, 不与主流程 journal 冲突**), status=completed (盘后补跑); git commit 并 push origin Main。
6. **安全前提**: 主流程按 §0.0 时段校验**永不盘后下单**, 故补跑是收盘后唯一下单者, 二者不会双重下单。
