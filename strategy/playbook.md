...
> | 段 | 时刻 | 跑什么 | 产出 |
> |---|---|---|---|
> | **① 收盘前关键路径** | **15:20 ET** (`daily.py --phase preclose`) | preflight → 取数 → 正股信号 → 期权信号 | 出场卖单**即时成交** + pending (首选窗 15:30–15:55, 未执行顺延次日 09:45–15:55) |
> | **② 收盘后收尾** | 17:45 ET (`daily.py --phase wrapup`) | 纸面轨道 + 行情管道核对 + journal + 账本回写 **+ 当日战报** (2026-09-11 并入, 原 18:45 独立窗口已停用) | 无实盘新单 |
>
...
- **时效字段一律照抄 `plan.json` 的 `to_pending.pending_template`** (2026-09-10 加, 起因当日
  ...
  两种形态:
  - **收盘前产出** (`exec_day: same_day_then_next_session`, 2026-09-11 用户选「顺延」):
    首选窗 **当日 15:30–15:55**; 未执行则**顺延**至**次一交易日 09:45–15:55**, 至次日 15:55 才过期。
    出场卖单已在收盘前即时成交、回款已到账。**顺延期内 status 保持 `awaiting_execution`, 不标 expired。**
...
2. **盘中单笔市价模式 (2026-09-10 用户批准, 取代原「盘外两腿混合执行」)** — 按 seq 顺序执行
   ...
   - **订单形态: 按触发时刻分两种模式 (2026-09-25 用户选「3」恢复盘外腿, 部分撤销 09-10 退役)**

     **(a) 盘中模式 — 首选**, 触发时刻在交易日 09:45–15:55 ET:
     单笔 `market` + `regular_hours` + `dollar_amount`, 一单一腿, 不拆腿。
     美元额市价单原生支持分数股 (原两腿拆分的唯一理由是 RH 拒绝分数股**限价**单, 市价单无此限制)。
     **零额外成本**, 滑点实测 −0.10%~+0.05%。能走盘中就绝不走盘外。

     **(b) 盘外模式 — 有成本的退路**, 触发时刻在 15:55 ET 后至次一交易日 09:45 ET (含周末):
     - 券商硬约束: `dollar_amount` 必须配 `type=market`, 而 market 只在 `regular_hours` 成交;
       分数股同理。**盘外只有整股限价单能成交** —— 这是恢复①腿的唯一原因。
     - **①腿 (整股限价)**: `limit_price = round(est_price × 1.010, 2)` (基准必须是**引擎 est_price**,
       不得改用实时价 — 红线2); `whole = floor(dollar_amount ÷ limit_price)`;
       `whole ≥ 1` → `place_equity_order(limit, whole 股, all_day_hours, gfd, ref_id)`;
       被拒则降级 `extended_hours` → `regular_hours`, 每次降级记 journal。
     - **`whole == 0`** (股价 > 单笔金额, 如 EQIX $1,014 vs 金额 $1,021.95 加容差后买不起 1 股):
       **该单盘外整单跳过**, 全额留给次日盘中窗。不得为凑整放大金额 (红线2)。
     - **零头 (remainder = dollar_amount − 实际成交额)**: **不下②腿** —— 旧②腿是"次日开盘市价单 +
       09:25 截止", 正是 2026-09-10 退役理由①的来源 (回款 09:30 到账, 清单 09:25 已过期,
       实测 32% 出单额结构性拿不到)。现在零头**随清单既有的顺延机制**走次日盘中窗 09:45–15:55,
       与未执行部分同一条路径, 不另起机制。
     - ⚠️ **成本**: ①腿实测多付 **0.55%** (中位 +0.59%, n=10, 2026-09-10 退役依据);
       盘后点差中位约 **2%** (2026-09-25 实测 CB 1.52% / ROKU 1.31% / EQIX 2.14% / ELV 2.21% / TRGP 2.97%)。
       **执行前必须向用户回显这一成本**, 并说明零头比例 (2026-09-25 实测零头占 29.8%)。
     - ⚠️ **1B 逐批确认要重做**: 盘外模式的订单形态 (整股/限价/数量) 与用户确认盘中清单时看到的
       (美元额/市价) **不同**, 且可能有单落空 —— 必须重新回显盘外逐笔明细并等用户确认, 不得沿用。

     > **恢复依据 (2026-09-25)**: 5 个交易日内越窗两次 (09-21 过 20.5 分, 09-25 过 18.6 分且跨周末),
     > 清单从产出 (15:22) 到关窗 (15:55) 只有 33 分钟。用户在三个方案 (①维持现状靠顺延 /
     > ②倒计时升级为硬闸 / ③恢复盘外腿) 中选「3」。退役理由①已由"零头走顺延"规避,
     > 理由②③ (多付 0.55%、运维复杂度) 依然成立, 故盘外仅作退路。
     > 退役全文与原②腿设计见 `strategy/archive/retired-offhours-two-leg.md`。
   - **执行窗口: 以清单自带的 `pending_template.exec_window_et` / `exec_day` 为准**
     (2026-09-10 拆分后有两种, 不要凭记忆):
     - **`same_day_then_next_session` (收盘前主跑产出; 2026-09-11 用户选「顺延」)**:
       首选窗当日 **15:30–15:55 ET** (= 12:30–12:55 PT) —— 出场卖单已在收盘前即时成交、
       回款已到账, 所以本形态**不需要等次日开盘**。
       **当日窗内未执行 → 不作废, 顺延至次一交易日 09:45–15:55 ET** (与 next_session 同窗),
       次日 15:55 仍未执行才改 `expired`。
       - **起因**: 收盘前窗仅 25 分钟, 2026-09-11 用户回复落在窗外, 4 单全部过期 0 成交
         (`journal/2026-09-11-preclose.md` §7)。顺延不缩短任何窗口, 只给漏掉的清单一条退路。
       - **不会重复买入**: 次一交易日收盘前主跑产出新清单时**覆写** `pending_orders.json`,
         旧清单自动消失, 不存在两份并存。
       - ⚠️ **顺延执行的是生成日的信号与金额** (逐字段引擎输出, 红线2 不得改),
         标的可能已不在当日最优之列 —— 执行前须向用户回显逐笔明细, 用户可一票否决任一单。
       - ⚠️ **资金必须重取**: 顺延到次日时实时 BP 已与生成时不同, 不得用 `buying_power_at_generation`。
